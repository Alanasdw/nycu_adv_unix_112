...
static void find_path( int path[], int *path_len, const maze_t *mz)
{
    ...
    while ( stack_top >= 0)
    {
        ...
        points[ current.depth] = current.pos;
    ...
    *path_len = stack[ stack_top].depth + 1;
    ...
    for ( int i = 1; i < *path_len; i += 1)
    {
        ...
        {
            if ( points[ i - 1].x + offset[ j].x == points[ i].x &&
                points[ i - 1].y + offset[ j].y == points[ i].y)
            {
                path[ i - 1] = j;
                break;
            }// if
...
maze_t *maze_load(const char *fn)
{
    ...
    */
    ...
    int max_offset = 0;
    int min_offset = 0xffffff;
    for ( int i = 0; i < sizeof( move_target) / sizeof(int); i += 1)
    {
    ...
    // printf("start_got: %p, last: %p\n", start_got, start_got + got_len);
